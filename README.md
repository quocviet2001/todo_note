...
4. **Cấu hình `.env`**: Bạn cần tạo một tệp `.env` để cấu hình các thông số như kết nối đến cơ sở dữ liệu MongoDB và khóa bí mật cho session.
   
         MONGO_URI=your_mongo_database_url
         SESSION_SECRET=your_session_secret
6. **Chạy ứng dụng**: `npm start` sẽ khởi động ứng dụng và `npm run watch` sẽ khởi động file css cho ứng dụng.
   Lưu ý: Bạn phải chạy song song 2 lệnh này trên Terminal.
7. Truy cập ứng dụng qua trình duyệt tại `http://localhost:3000/todo-note`.
